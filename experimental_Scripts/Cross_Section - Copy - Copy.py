...
search_fields = ["POINT_X","POINT_Y","OID@","LineOID","SHAPE@"]
    ...
    with arcpy.da.InsertCursor(cross_section_points, (insert_fields)) as insert_point:
            with arcpy.da.InsertCursor(cross_section_lines, (insert_fields_lines)) as insert_line:
                for row in search:
                        ...
                        if row[2]!=1:
                                this_point=row[4]
                                direction=calcGeomCardinality(last_point,this_point)
...
#extract elevation values to cross section _points
inRasterlist=[[input_dem]]
arcpy.AddMessage("\t\tExtracting values to points...")
ExtractMultiValuesToPoints(cross_section_many_points,inRasterlist,"NONE")

search_fields = ["SHAPE@", "PointOID","LineOID", "Value","redo_boone_dem"]
objects=set()
